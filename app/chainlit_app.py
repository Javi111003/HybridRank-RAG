...
async def start():
    try:
        ...
        welcome = (
            f"**Sistema HybridRank RAG listo**\n\n"
            ...
            f"Escribe tu consulta sobre legislacion cubana."
        )
        msg.content = welcome
        await msg.update()
...
async def handle_message(message: cl.Message):
    ...
    try:
        ...
        # Fragmentos recuperados como elementos expandibles
        if result.fragments:
            elements = []
            for i, frag in enumerate(result.fragments[:5], start=1):
            ...
            await cl.Message(
                content=f"Se recuperaron {len(result.fragments)} fragmentos. "
                f"Ver fuentes en el panel lateral.",
                elements=elements,
            ).send()
